...
class Startup:
    ...
    def check_updates(self):
           if current_build == 'No Build Installed':
               print('###: There is currently no build installed.\nWould you like to install one now?')
           ...
           if version > current_version and setting('update_passed') != 'true':
               ...
               if update_available == 1:
                   print('### check ver co update')
                   xbmc.executebuiltin(f'ActivateWindow(10001, "plugin://{addon_id}/?mode=1",return)')
               elif update_available == 0:
                   print('### check ver ko can update')
    ...
    def run_startup(self):
        print('### run_startup')
        print('### binaries_path:', binaries_path)
        if binaries_path.exists():
            print('### restore_binary')
            restore_binary()
        if setting('autoclearpackages')=='true':
            print('### clear_packages')
            xbmc.sleep(2000)
            clear_packages()
        ...
        print('### will call notify_check')
        self.notify_check()
        print('### will call check_updates')
        self.check_updates()
